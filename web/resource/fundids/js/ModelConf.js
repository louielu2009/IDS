...
$(function(){	            
              ...
              //************元素区域点击触发事件************//
              $(".Model_Element").live("click",function(){                                         
                                           $(".Model_Element").removeClass("Model_Element_Active");
                                           $(this).addClass("Model_Element_Active");
                                           return false;
                                        });	            
	            //********元素编辑框配置修改激活事件*********//
	            $("input,select",".Element_Config").live("change",function(e){																							
	            ...
	            $(".Model_Relocate").live("click",function(){
	            	                         ...
	            	                         var _top = $obj.offset().top-$(".sys_content").offset().top+$(".sys_content").scrollTop();	            	                         
	            	                         if(_top!=0)
	            	                         $(".sys_content").scrollTop(_top);	            	                         
	            	                       });	             	                                   	            
	            ...
	            $(".Model_LeftBox_SearchButton").live("click",function(){
                       ...
                       $(".Model_Element").each(function(){
 	                         var Name = $(this).find(".Model_Element_FirstLine").text();
 	                         if(Name.indexOf(str)!=-1)
 	                         $(this).addClass("Model_Element_Find");	            	                                           	                         
 	                      });         	            	
                   });
               //**********左边搜索功能**********//    
              /* $(".Model_LeftBox_SearchButton").live("click",function(){
               ...
               //***Enter键搜索***//
	            ...
	            $(".Model_Element_Title").live("click",function(e){	            	                                    	                        
	            	                       ...
	            	                       .live("blur",function(){
	            	                        if($(this).attr("value")==$(this).attr("originValue"))
	            	                        {
	            	                          var htm = '<span class="Model_Element_Title">'+$(this).val()+'</span>';
	            	                          $(this).replaceWith(htm);
	            	                          return;	            	                        
	            	                        }
...
function Element_outHtml(data){	                        
                          ...
                          {                           
                            $(".Model_Element_Active").replaceWith(r[2]);                           
...
function chartY_mgr(obj,flag)
{
	...
	if(0==flag){//删除
		//alert($(obj).parents("tbody:first").html());
		if(trL<=2) return alert("必须有Y轴存在！");
		$(obj).parents("tr:first").remove();
		//删除更多?
		//$(obj).parents("tr").find(".itemMore").tsForm("showSubForm",{"data":""}) ;
	}
	if(1==flag){//增加
     ...
     //2 “更多”的处理，在轴第8行：$('body').on("change",'select[chartproperty="axisSelect"]',function(){
      //var subContent= subForm_conf['content']['Content'];
      var subContent= subForm_conf["conf"]['content'];     
      ...
      //subForm_conf['content']['Content']= subContent;
		  ...
		  var _tr = $(obj).parents("tbody:first").find("tr:last") ;
			...
			if($(_tr).find(".itemMore").length){
     		 ...
     		 $(_new_more_obj).attr("id",_newSub_id) ;
				 ...
				 var subContent= subForm_conf['conf']['content'] ;//subForm_conf['content']['Content']
				 
      for(var i=0;i<subContent.length;i++){
      	
      		var subCon2; 
      		subContent[i][0]['attr']?
      		subCon2=subContent[i]:
      		subCon2=subContent[i][0];
      		
      		for(var j=0;j<subCon2.length;j++){
      			var attr=subCon2[j]['attr'];
      			var a= 	attr['property'];
      			...
      			var proArr= a.split(':');
						...
						//alert($.toJSON(subContent))
						subContent[i][0][j]['attr']?
						subContent[i][0][j]['attr']['property']= newPro:
						subContent[i][j]['attr']['property']= newPro;
      		}
      };
      subForm_conf['content']= subContent;
...
$().ready(function(){
	$(".modelMgrTable tr").live({
		"mouseenter":function(){
			var fstr= '<span class="button_span"><button class="btn" onclick="return model_mgr(this,1);">打开</button><button class="btn" onclick="return model_mgr(this,2);">数据</button><button class="btn" onclick="return model_mgr(this,3);">报告</button><button  class="btn" onclick="return model_mgr(this,4);">';
...
//左边文档搜索
function LeftBoxSearch(){
	var SearchKey = $(".Model_LeftBox_SearchContent").val();
  var regExp = new RegExp(SearchKey,"gi"); 
  $(".Model_LeftBox_TitleList li").each(function(){		
					 var txt = $(this).text();	 					
				  // alert(txt);
